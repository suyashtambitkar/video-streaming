...
const SearchResult = () => {
    const { videoSearch } = useParams();
    const [videos, setVideos] = useState();
    const navigate = useNavigate();

    useEffect(() => {
        const getResult = async () => {
            const res = await makeAuthGetRequest(`${API_URL}/video/search/${videoSearch}`);
            setVideos(res);
        }
        if (videoSearch) {
            getResult();
        ...
        <Generalised>
            <div className="h-full w-full px-3">
                <div className="h-[8%] w-full">
                    <h3 className="py-2 px-4 bg-[#181717]"> Search Result</h3>
                </div>
                ...
                <div className="h-[92%] w-full px-2 overflow-y-scroll scrollbar-thin scrollbar-track-rounded-full scrollbar-thumb-[#2f2f32] scrollbar-track-[#000]">
                    {
                        ...
                        ) : (<p className="text-[#909297] text-sm">No video Found.</p>)
                    }
